...
void savePicture( const std::string& filename, int W, int H, int* nbIters, int maxIter )
{
...
int main(int argc, char *argv[] ) 
 { 
	...
	int hauteur = H / nbp;
	int const lenp = W * hauteur;
	int num_ligne = rank * H / nbp;
	int pixels[lenp];
	computeMandelbrotSetRow(W, hauteur, maxIter, num_ligne, pixels);
	if (rank == 0) {
		rbuff = (int *)malloc(W*H*sizeof(int));
	}
	MPI_Gather(pixels, lenp, MPI_INT, rbuff, lenp, MPI_INT, 0, globComm);
    savePicture("mandelbrot.tga", W, H, rbuff, maxIter);
	MPI_Finalize();
    return EXIT_SUCCESS;
 }
